...
function gadget:GetInfo()
	return {
		...
		desc      = "Handles Taxi requests, Mass/Size validation, and UI communication.",
...
function gadget:CommandFallback(unitID, unitDefID, teamID, cmdID, cmdParams, cmdOptions)
	...
	-- If unit is in the air, we are successfully transporting
	...
	-- Arrival Check
	if ((ux-tx)^2 + (uz-tz)^2) < (40*40) then
		unitsWaiting[unitID] = nil
		SpSetUnitRulesParam(unitID, "waiting_for_taxi", 0, {public = true})
		return true, true 
	end

	-- Initialization of Wait State
	...
	if unitsWaiting[unitID].assignedTaxi then
		...
		if not Spring.ValidUnitID(taxi) or Spring.GetUnitIsDead(taxi) or #(SpGetUnitCommands(taxi, 1) or {}) == 0 then
			...
			SpSetUnitMoveGoal(unitID, tx, ty, tz) -- Resume walking
		else
			SpSetUnitMoveGoal(unitID, ux, uy, uz) -- Stay for pickup
		end
	end
	return true, false
end
	...
	for unitID, data in pairs(unitsWaiting) do
		if not data.assignedTaxi then
			...
			for i = 1, #teamUnits do
				...
				-- Transport must be idle and capable of lifting this specific unit
...
function gadget:UnitCreated(unitID, unitDefID)
	if transportableUnits[unitDefID] then
		Spring.InsertUnitCmdDesc(unitID, {
			...
			name="Transport To", 
			action="transport_to", 
			tooltip="Request air transport to destination.", 
			cursor="Transport"
		})
	end
end
